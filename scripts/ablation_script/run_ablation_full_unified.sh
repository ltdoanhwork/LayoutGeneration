...
if [ "$DRY_RUN" != "--dry-run" ]; then
    echo ">> Checking/Preparing Datasets (Group 6)..."
    bash scripts/precompute_script/run_dataset_ablation_prep.sh $GPU_ID
    
    echo ">> Checking/Preparing prompt attributes (Group 7)..."
    # Generate for 1, 2, 3, 4, 5 pairs
    for N in 1 2 3 4 5; do
        if [ ! -f "data/sakuga_dataset_v11_new/anime_attrs_${N}pair.npy" ]; then
            echo "Generating attributes for ${N}-pair prompts..."
             "$PYTHON" scripts/precompute_script/prepare_anime_attrs_ablation.py \
                --dataset_dir "data/sakuga_dataset_v11_new" \
                --num_pairs $N \
                --device cuda
        fi
        if [ ! -f "data/sakuga_dataset_v11_new_test/anime_attrs_${N}pair.npy" ]; then
             "$PYTHON" scripts/precompute_script/prepare_anime_attrs_ablation.py \
                --dataset_dir "data/sakuga_dataset_v11_new_test" \
                --num_pairs $N \
                --device cuda
        fi
    done
...
# ============================================================
# GROUP 7: PROMPT SENSITIVITY (New)
# ============================================================
echo -e "\n====== GROUP 7: PROMPT SENSITIVITY ======"

# 1-Pair
run_exp "G1_prompt_1pair" $BASE_ARGS --attr_suffix "anime_attrs_1pair.npy"

# 2-Pair 
run_exp "G1_prompt_2pair" $BASE_ARGS --attr_suffix "anime_attrs_2pair.npy"

# 3-Pair (Baseline - should match 1_rec_opt_ours)
run_exp "G1_prompt_3pair" $BASE_ARGS --attr_suffix "anime_attrs_3pair.npy"

# 4-Pair
run_exp "G1_prompt_4pair" $BASE_ARGS --attr_suffix "anime_attrs_4pair.npy"

# 5-Pair
run_exp "G1_prompt_5pair" $BASE_ARGS --attr_suffix "anime_attrs_5pair.npy"
