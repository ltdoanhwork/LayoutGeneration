...
DEFAULT_MEDIAL_RIDGE_THRESHOLD = getattr(sd, "DEFAULT_MEDIAL_RIDGE_THRESHOLD", 0.39)
...
def build_medial_graph(multilinestring, line_labels, distance, small_branch = 8, connecting_mode=False):
    """Mirror the shared shape_decomposition graph builder, then add edge lengths."""
    G = sd.build_medial_graph(
        multilinestring,
        line_labels,
        distance,
    ...
    for u, v in G.edges():
        x_1 = G.nodes[u].get('x', 0.0)
        y_1 = G.nodes[u].get('y', 0.0)
        x_2 = G.nodes[v].get('x', 0.0)
        y_2 = G.nodes[v].get('y', 0.0)
        G.edges[u, v]['weight'] = math.hypot(x_1 - x_2, y_1 - y_2)
...
def extract_foreground(label):
    ...
    if foreground_area > total_area / 200:
        ...
        y1 = int(np.min(np.where(foreground)[0]))
        y2 = int(np.max(np.where(foreground)[0]))
    else:
    ...
    return x1, y1, x2, y2, foreground_exist

def process_image_for_optimization(image_dict):
    index = 0
    images = []
    for idct in image_dict:
        # foreground is unified as [x1, y1, x2, y2]
        aspect_ratio = (idct['foreground'][2]-idct['foreground'][0])/(idct['foreground'][3]-idct['foreground'][1] + 1e-6)
...
def optimization(input_shape, input_mask_folder, output_dir, 
    ...
    Args:
        ...
        ablation_mode: Ablation experiment mode (wo_cap, wo_asp, wo_ov, only_cap, only_asp, only_ov, or None for full)
    """
    ...
    if use_voronoi_layout:
        ...
        if summary_json and os.path.isfile(summary_json):
            ...
            for mask_file in mask_files:
                ...
                x1, y1, x2, y2, _ = extract_foreground(label)
                h, w = label.shape[:2]
                
                # Unified bbox format: [x1, y1, x2, y2]
                bbox = [x1, y1, x2, y2]
        ...
        if use_timeline_order:
            print("  [ASSIGN] Timeline sorting kept for metadata only; image-cell matching is free.")
        else:
            print("  [ASSIGN] Free image-cell assignment enabled; no timeline/probability reorder applied.")
        
        print(f"[SAS] Processing {len(frame_infos)} frames with Voronoi layout")
        
        # Set ablation mode in CONFIG BEFORE creating the engine
        if ablation_mode is not None:
            from voronoi_layout import CONFIG
            CONFIG['ablation_mode'] = ablation_mode
            print(f"[SAS] Ablation mode set to: {ablation_mode}")
        
        # Create Voronoi layout (will use updated CONFIG)
        ...
        parts_dict, unbuffered_dict, mapping, cuts = convert_voronoi_to_slicing_format(layout_result)
        
        # Convert parts_dict to list format with proper structure
        # collage_assembly.py expects: {'coords': [...], 'foreground': [x1, y1, x2, y2]}
        
        parts_list = []
        old_to_new_part_idx = {}
        skipped_empty_invalid = 0
        ...
        for i in sorted(parts_dict.keys()):
            ...
            # Store both buffered coords (for rendering) and unbuffered coords (for evaluation)
            ub_coords = unbuffered_dict.get(i, [])
            parts_list.append({
                'coords': coords,
                'coords_unbuffered': ub_coords if ub_coords and len(ub_coords) >= 3 else coords,
                'foreground': [fg_x1, fg_y1, fg_x2, fg_y2]
            })
        ...
        print(
            f"[SAS] Cell export summary: valid={len(parts_list)}, "
            f"empty_or_invalid={skipped_empty_invalid}, degenerate={skipped_degenerate}"
        )
        ...
        for part in parts_list:
            fg = part['foreground']
            part_centers.append(((fg[0] + fg[2]) * 0.5, (fg[1] + fg[3]) * 0.5))
        ...
        unique_parts = len({im['assigned_part'] for im in image_dict})
        duplicate_assignments = len(image_dict) - unique_parts
        print(
            f"[SAS] Assignment summary: images={len(image_dict)}, unique_parts={unique_parts}, "
            f"duplicate_assignments={duplicate_assignments}, remapped={remap_count}"
        )
        ...
        optimization_output = {
            ...
            "cuts": cuts,
            "layout_type": "voronoi",
            "initial_site_centroids": layout_result.get("initial_site_centroids", []),
        }
    ...
    for image_id in image_ids:
        ...
        x1, y1, x2, y2, foreground_exist = extract_foreground(label)
        it = image_template.copy()
        it["filename"] = image_id + ".png"
        it["foreground"] = [x1, y1, x2, y2]
