...
# Voronoi pipeline does not need final_cut.json; legacy grid mode in sas_optimization reads it if present.
WRITE_FINAL_CUT_JSON = False

# Keep the medial-axis ridge threshold aligned with the Voronoi pipeline.
DEFAULT_MEDIAL_RIDGE_THRESHOLD = 0.39
...
def generate_canvas_polygon(img, complement=False):
    ...
    # Keep OpenCV image coordinates (top-left origin, y-down) consistently.
...
def ridge_medial_axis(image, ridge_threshold=DEFAULT_MEDIAL_RIDGE_THRESHOLD, small_threshold=5, component_max_distance=15, background_size=40000):
...
def rowcol2xy(row, col, ymax):
    # Unified image coordinates: x=col, y=row (top-left origin).
    return int(col), int(row)

def xy2rowcol(x, y , ymax):
    # Unified image coordinates: row=y, col=x (top-left origin).
...
def redistribute_vertices(geom, gap):
    if geom.geom_type == 'LineString':
        ...
        # Handle GeometryCollection (common for simple shapes like squares)
        # Extract only LineString and MultiLineString geometries
        valid_parts = []
        for g in geom.geoms:
            if g.geom_type in ('LineString', 'MultiLineString', 'LinearRing'):
                redistributed = redistribute_vertices(g, gap)
                if not redistributed.is_empty:
                    valid_parts.append(redistributed)
        
        if not valid_parts:
            return MultiLineString([])
        elif len(valid_parts) == 1 and valid_parts[0].geom_type == 'MultiLineString':
            return valid_parts[0]
        else:
            # Flatten all parts into MultiLineString
            lines = []
...
MIN_BOUNDARY_ARC_EPS = 1e-3
MIN_CUT_CHORD_EPS = 1e-3
BOUNDARY_PARAM_QUANT_EPS = 1e-3


def _cut_thresholds(boundary_linestring):
    total_len = float(boundary_linestring.length)
    min_arc = max(MIN_BOUNDARY_ARC_EPS, total_len * 1e-4)
    min_chord = max(MIN_CUT_CHORD_EPS, total_len * 1e-4)
    quant = max(BOUNDARY_PARAM_QUANT_EPS, total_len * 1e-4)
    return min_arc, min_chord, quant


def _order_pair_cw(boundary_vertices, index_1, index_2):
    if is_cw(boundary_vertices, index_1, index_2):
        return [index_1, index_2]
    return [index_2, index_1]


def is_valid_cut_geometry(vertex_1, vertex_2, boundary_linestring, min_arc=None, min_chord=None):
    if min_arc is None or min_chord is None:
        inferred_arc, inferred_chord, _ = _cut_thresholds(boundary_linestring)
        min_arc = inferred_arc if min_arc is None else min_arc
        min_chord = inferred_chord if min_chord is None else min_chord

    euclidean_distance = math.hypot(vertex_1[0] - vertex_2[0], vertex_1[1] - vertex_2[1])
    if euclidean_distance <= min_chord:
        return False

    return minimal_arc(vertex_1, vertex_2, boundary_linestring) > min_arc


def _is_valid_projection_indices(boundary_vertices, index_1, index_2, min_arc, min_chord):
    coords = list(boundary_vertices.coords)[:-1]
    length = len(coords)
    if length < 2:
        return False
    i1 = int(index_1) % length
    i2 = int(index_2) % length
    if i1 == i2:
        return False
    return is_valid_cut_geometry(coords[i1], coords[i2], boundary_vertices, min_arc=min_arc, min_chord=min_chord)


def canonical_cut_key(cut, boundary_linestring, quant=None):
    if quant is None:
        _, _, quant = _cut_thresholds(boundary_linestring)
    s1 = boundary_linestring.project(Point(cut[0]))
    s2 = boundary_linestring.project(Point(cut[1]))
    lo, hi = sorted((s1, s2))
    return (int(round(lo / quant)), int(round(hi / quant)))
...
def find_projection_pair(boundary_vertices, x, y, projection_distance, interior=True):
    ...
    min_arc, min_chord, _ = _cut_thresholds(boundary_vertices)
    
    def vector_angle(vector_1, vector_2):
        n1 = np.linalg.norm(vector_1)
        n2 = np.linalg.norm(vector_2)
        if n1 < 1e-12 or n2 < 1e-12:
            return 0.0
        vector_1 = vector_1 / n1
        vector_2 = vector_2 / n2
    ...
    if projection_distance < single_point_projection_threshold: # too close to boundary
        ...
        boundary_len = max(float(boundary_vertices.length), 1e-6)
        avg_step_len = boundary_len / max(length, 1)
        target_arc = max(single_point_projection_threshold, projection_distance * 1.25, 2.0 * avg_step_len)
        base_offset = int(round(target_arc / max(avg_step_len, 1e-6)))
        base_offset = max(1, min(base_offset, max(length // 2, 1)))
        offsets = sorted({
            base_offset,
            max(1, base_offset // 2),
            min(max(length // 2, 1), base_offset + 1),
        }, reverse=True)

        for offset in offsets:
            i1 = (nearest - offset) % length
            i2 = (nearest + offset) % length
            if _is_valid_projection_indices(boundary_vertices, i1, i2, min_arc, min_chord):
                return _order_pair_cw(boundary_vertices, i1, i2)
        return []
    ...
    for projection_pair in itertools.combinations(possible_projections, 2):
        if not _is_valid_projection_indices(boundary_vertices, projection_pair[0], projection_pair[1], min_arc, min_chord):
            continue
    ...
    return _order_pair_cw(boundary_vertices, current_pair[0], current_pair[1])
...
def mark_component(boundary_vertices_dict):
    ...
    keys = list(boundary_vertices_dict.keys())
    if not keys:
        return copied
    ...
    # back propagation
    if isinstance(copied[keys[0]], Component):
        for i in range(-1, -len(keys) - 1, -1):
            if isinstance(copied[keys[i]], Corner):
                break
            if isinstance(copied[keys[i]], Component):
                copied[keys[i]] = copied[keys[0]]
...
def extract_projection_pair(G, boundary_vertices, return_source = False):
    ...
    for v in G.nodes:
        ...
        projections = find_projection_pair(boundary_vertices, medial_vertex_x, medial_vertex_y, medial_vertex_distance)
        if len(projections) == 2:
            cut = (coords[projections[0]], coords[projections[1]])
            if is_valid_cut_geometry(cut[0], cut[1], boundary_vertices, min_arc=min_arc, min_chord=min_chord):
                projection_pairs.append(cut)
                sources.append(v)
    if return_source:
...
def generate_raw_cuts(projection_pairs, corner_dict, boundary_vertices):
    raw_cuts = []
    seen = set()
    min_arc, min_chord, quant = _cut_thresholds(boundary_vertices)

    for pp in projection_pairs:
        if not (isinstance(corner_dict[pp[0]], Corner) or isinstance(corner_dict[pp[1]], Corner)):
            continue
        if not is_valid_cut_geometry(pp[0], pp[1], boundary_vertices, min_arc=min_arc, min_chord=min_chord):
            continue

        key = canonical_cut_key(pp, boundary_vertices, quant=quant)
        if key in seen:
            continue
        seen.add(key)
        raw_cuts.append(pp)
...
def protrusion_strength(vertex_1, vertex_2, boundary_linestring):
    ...
    if min_arc <= 0:
        raise ValueError(
            f"Invalid cut reached protrusion_strength: v1={vertex_1}, v2={vertex_2}, min_arc={min_arc}"
        )
...
def generate_cuts(filename, output_path):
    ...
    ma_ext = ridge_medial_axis(
        medial_exterior_input,
        ridge_threshold=DEFAULT_MEDIAL_RIDGE_THRESHOLD,
        small_threshold=5,
    )
    ma_int = ridge_medial_axis(
        medial_interior_input,
        ridge_threshold=DEFAULT_MEDIAL_RIDGE_THRESHOLD,
        small_threshold=5,
    )
    ...
    if ma_ext[0].sum() > 0:
        ...
        raw_cuts = generate_raw_cuts(projection_pairs, component_adjusted_ext, boundary_vertices)

        # Skip plotting due to descartes compatibility issue with Shapely 2.x
        ...
        # Skip plotting due to descartes compatibility issue
        # plot_raw_cuts(representative, polygon, image.shape[1], image.shape[0])
