...
1. Constrained RL with Lagrangian optimization
2. State-dependent gating (alpha_t per frame)
3. PCGrad gradient surgery for conflict-free updates
4. DPP two-stage selection  
5. Quantile-based rewards for outlier hunting
6. Test-time scaling: sample N → rerank → select best
7. Comprehensive visualization and logging
...
from src.rl.premium_rewards_v8 import (
    PremiumRewardV8, 
    create_reward_system_v8,
    ConstraintConfig,
    DPPConfig,
...
class PPOTrainerV8:
    ...
    def train_step(
        ...
        reward_rec = -rewards["constraint_penalty"]  # Rec task minimizes constraint violations
        reward_anime = rewards["anime"]               # Anime task maximizes aesthetic quality
...
def main():
    ...
    ap.add_argument("--diversity_threshold", type=float, default=0.25)
    ...
    # Reward scales
    ap.add_argument("--anime_scale", type=float, default=3.0)
    ap.add_argument("--quantile_scale", type=float, default=2.0)
    ...
    config["version"] = "V8"
    with open(os.path.join(args.save_dir, "config_v8.json"), "w") as f:
        json.dump(config, f, indent=2)
    
    print("=" * 70)
    print("V8 Constrained Multi-Objective RL Training")
    print("=" * 70)
    print(f"Features: PCGrad={bool(args.use_pcgrad)}, DPP={bool(args.use_dpp)}, TTS={bool(args.use_tts)}")
    print(f"Constraints: RecErr≤{args.rec_err_threshold}, Coverage≤{args.coverage_threshold}, Diversity≥{args.diversity_threshold}")
    print(f"Lagrangian LR: {args.lambda_lr}")
    print("=" * 70)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Save checkpoint
        checkpoint_path = os.path.join(args.save_dir, f"dsn_v8_ep{epoch}.pt")
        torch.save({
            "epoch": epoch,
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": base_optimizer.state_dict(),
            "lagrangian_state": reward_system.get_lagrangian_state(),
            "config": config,
        }, checkpoint_path)
