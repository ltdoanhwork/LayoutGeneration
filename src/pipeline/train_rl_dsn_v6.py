#!/usr/bin/env python3
"""
V6 Multi-Task RL Training Script - Quality-Aligned

Key improvements over V5:
1. Uses PremiumAnimeRewardV4 with continuous quality rewards
2. Per-attribute rewards that MATCH evaluation metrics
3. Top-k coverage rewards to ensure best frames selected
4. Balanced reward scaling between RecErr and Anime tasks
5. Enhanced TensorBoard logging with per-attribute metrics
6. Quality improvement tracking aligned with evaluation

Usage:
    python -m src.pipeline.train_rl_dsn_v6 --dataset_root data/sakuga_dataset_100_samples ...
"""
...
from src.rl.premium_rewards_v4 import PremiumAnimeRewardV4, ATTR_NAMES
...
# ---------------- Main Training Loop ---------------- #
def main():
    ap = argparse.ArgumentParser(description="V6 Multi-Task RL Training (Quality-Aligned)")
    ...
    ap.add_argument("--anime_reward_scale", type=float, default=2.5, 
                    help="Scale factor for anime rewards to match RecErr magnitude")
    ap.add_argument("--top_k_ratio", type=float, default=0.1,
                    help="Fraction of frames considered high-quality (top-k)")
    ap.add_argument("--use_continuous_rewards", type=int, default=1,
                    help="Use continuous quality rewards (1) or binary (0)")
    ...
    # Logging
    ap.add_argument("--log_dir", type=str, default="runs/dsn_v6/logs")
    ...
    ap.add_argument("--save_visualizations", type=int, default=1,
                    help="Save validation visualizations to output dir")
    ...
    # Create outputs dir for visualizations
    ...
    print("=" * 60)
    print("V6 Multi-Task RL Training (Quality-Aligned)")
    print("=" * 60)
    print(f"[V6] Tasks: RecErr + Anime (quality-aligned)")
    print(f"[V6] Anime reward scale: {args.anime_reward_scale}")
    print(f"[V6] Top-k ratio: {args.top_k_ratio}")
    print(f"[V6] Continuous rewards: {bool(args.use_continuous_rewards)}")

    # V6 Quality-Aligned Reward System
    premium_reward = PremiumAnimeRewardV4(
        top_k_ratio=args.top_k_ratio,
        use_continuous=bool(args.use_continuous_rewards),
    ...
    hparams_dict["algorithm"] = "Multi-Task PPO V6"
    hparams_dict["architecture"] = "DSNMultiTask"
    hparams_dict["reward_system"] = "PremiumAnimeRewardV4"
    writer.add_text('hyperparameters', json.dumps(hparams_dict, indent=2), 0)

    best_metric = None

    # Training loop
    epoch_pbar = tqdm(range(1, args.epochs + 1), desc="V6 Quality-Aligned Training", position=0)
    
    for epoch in epoch_pbar:
        np.random.shuffle(all_scene_dirs)
        
        # Reset reward tracker for new epoch
        premium_reward.reset_tracker()
        
        # Collect experiences
        experiences = []
        ep_rewards_rec = []
        ep_rewards_anime = []
        ep_quality_improvements = {name: [] for name in ATTR_NAMES}
        ep_top_k_recalls = []
        ep_top_k_precisions = []
        ...
        for scene_dir in rollout_pbar:
            ...
            if sample.anime_attrs is not None:
                ...
                feats = feats_clip
                attrs = np.zeros((len(feats_clip), 6), dtype=np.float32)
            ...
            # Task 2: Anime reward with V4 quality-aligned rewards
            ...
            anime_rewards = premium_reward.compute_reward(
                attrs_for_reward, sel_idx, current_epoch=epoch, return_details=True
            )
            
            # Scale anime reward to match RecErr magnitude
            R_anime = anime_rewards["total"] * args.anime_reward_scale
            ...
            for name in ATTR_NAMES:
                ep_quality_improvements[name].append(anime_rewards.get(f"improvement_{name}", 0.0))
            ep_top_k_recalls.append(anime_rewards.get("top_k_recall", 0.0))
            ep_top_k_precisions.append(anime_rewards.get("top_k_precision", 0.0))
        ...
        mean_top_k_recall = np.mean(ep_top_k_recalls) if ep_top_k_recalls else 0.0
        mean_top_k_precision = np.mean(ep_top_k_precisions) if ep_top_k_precisions else 0.0
        
        tqdm.write(f"[Epoch {epoch}] R_rec={mean_rec:.4f} | R_anime={mean_anime:.4f} | α={merge_weight:.3f}")
        tqdm.write(f"  Top-k: recall={mean_top_k_recall:.3f}, precision={mean_top_k_precision:.3f}")
        ...
        writer.add_scalar("train/top_k_recall", mean_top_k_recall, epoch)
        writer.add_scalar("train/top_k_precision", mean_top_k_precision, epoch)
        
        # Per-attribute quality improvements
        for name in ATTR_NAMES:
            vals = ep_quality_improvements[name]
            if vals:
                mean_imp = np.mean(vals)
                writer.add_scalar(f"train/quality_improvement_{name}", mean_imp, epoch)
        ...
        if args.val_videos_dir and args.val_output_dir and (epoch % args.validate_every == 0):
            ...
            if r.returncode != 0:
                ...
                if summary_path.exists():
                    ...
                    # Log anime quality metrics
                    anime_metrics = s.get("anime_quality_metrics", {})
                    for k, v in anime_metrics.items():
                        if v is not None:
                            writer.add_scalar(f"val/anime_{k}", float(v), epoch)
            ...
            if args.save_visualizations:
                viz_cmd = [
                    ...
                    "--output_dir", str(outputs_dir / "visualizations"),
                    "--save_images",
                ]
                tqdm.write("[Visualize] Generating validation plots...")
                viz_r = subprocess.run(viz_cmd, capture_output=True, text=True)
                if viz_r.returncode != 0:
                    tqdm.write(f"[Visualize][Warning] {viz_r.stderr[:200]}")
    ...
    print(f"📊 Visualizations saved to: {outputs_dir / 'visualizations'}")
    print(f"🎉 V6 Quality-Aligned Training Complete!")
    print("=" * 60)
